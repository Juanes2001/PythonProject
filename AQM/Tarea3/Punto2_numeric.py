...
idx = np.argsort(En)          # indices that sort eigenvalues
En = En[idx]                # sorted eigenvalues
bn = bn[:, idx]
...
ani = animation.FuncAnimation(fig2, animate,init_func=init, frames=70, interval=70, blit=True)
...
for l in range(5):
    ax3.plot(xvals,(phin[:,l:l+1].T)[0]+l)
