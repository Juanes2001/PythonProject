...
def  V(i,j):
    Vij_num = fn.dcroc(i,j)*np.where(np.abs(xvals[i] - 20) <= 1, Ebar, 0)
    return Vij_num

def pot(x):
    return np.where(np.abs(x - 20) <= 1, Ebar, 0)
...
ani = animation.FuncAnimation(fig2, animate,init_func=init, frames=150, interval=70, blit=True)
